...
class MainActivity3 : AppCompatActivity() {
    ...
    private fun saveNew() {
        ...
        val url = Uri.parse(Config.URL + "sensors")
            .buildUpon()
            .build().toString()
        val peticion = object: JsonObjectRequest(Method.POST, url, null,{
                response ->
            Toast.makeText(this, "Sensor guardado", Toast.LENGTH_LONG).show()
            finish()
        }, { error ->
            Toast.makeText(this, error.hashCode().toString(), Toast.LENGTH_LONG).show()
        }){
    ...
    private fun saveChanges() {
        ...
        val url = Uri.parse(Config.URL + "sensors/" + tvNewId.text)
            .buildUpon()
            .build().toString()
        val peticion = object: JsonObjectRequest(Method.PUT, url, null,{
                response ->
            Toast.makeText(this, "Sensor guardado", Toast.LENGTH_LONG).show()
            finish()
        }, { error ->
